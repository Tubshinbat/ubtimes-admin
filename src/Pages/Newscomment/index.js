...
const NewsComment = (props) => {
  ...
  const [columns, setColumns] = useState([
...

    {
      ...
      sorter: (a, b) => handleSort(),
      render: (text, record) => {
        return record.comment.substr(0, 170);
      },
    },
    {
      dataIndex: "news",
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/newscomments/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteNewsComment(selectedRowKeys);
  };
    ...
    switch (modal) {
      ...
      case "edit": {
        if (selectedRowKeys && selectedRowKeys.length === 1) {
          props.history.replace("/newscomments/edit/" + selectedRowKeys[0]);
        } else {
          toastControl("error", "Нэг өгөгдөл сонгоно уу");
        }
        break;
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("newscomments/excel?" + query);
    let excelData = [];
    if (response) {
  ...
  return (
    <>
      <div className="content-wrapper">
        <PageTitle name="Нийтлэлийн сэтгэгдэл" />
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Бүх нийтлэлийн сэтгэгдэл</h3>
              </div>
...
export default connect(mapStateToProps, mapDispatchToProp)(NewsComment);
